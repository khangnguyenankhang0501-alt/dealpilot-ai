...
export async function generateMetadata({
  ...
  const description =
    ...
    "Find the latest " +
      storeName +
      " coupons, promo codes, deals, and discounts on DealPilot.";
...
export default async function StorePage({
    ...
    return (
      <main className="mx-auto w-full max-w-6xl px-4 py-10 sm:px-6">
        ...
        <div className="mt-8 rounded-3xl border border-slate-200 bg-white px-6 py-16 text-center shadow-sm">
          ...
          <Link
            href="/stores"
            className="mt-6 inline-flex h-11 items-center justify-center rounded-xl bg-emerald-500 px-5 text-sm font-black text-white shadow-[0_8px_20px_rgba(16,185,129,0.20)] transition hover:-translate-y-0.5 hover:bg-emerald-400"
          >
            Browse Stores →
          </Link>
        </div>
  ...
  const { data: coupons, error: couponsError } = await supabase
    .from("coupons")
    .select("*, stores!coupons_store_id_fkey(id, name, slug, logo_url)")
    .eq("store_id", store.id)
    .eq("status", "Active")
    .or("expires_at.is.null,expires_at.gte." + today)
    .order("created_at", {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
      ...
      <section className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.05)]">
        <div className="flex flex-col gap-5 p-5 sm:p-6 md:flex-row md:items-center md:justify-between">
          <div className="flex min-w-0 items-center gap-4">
            <div className="relative flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm sm:h-20 sm:w-20">
              {store.logo_url ? (
                <Image
                  src={store.logo_url}
                  alt={storeName}
          ...
          <Link
            href="/stores"
            className="inline-flex h-10 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-xs font-bold text-slate-700 transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700 sm:text-sm"
          >
            All Stores
          </Link>
        </div>
      ...
      <section className="mt-10">
        ...
        {coupons && coupons.length > 0 ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-5 py-14 text-center">
            ...
            <Link
              href="/coupons"
              className="mt-5 inline-flex h-10 items-center justify-center rounded-xl bg-emerald-500 px-4 text-xs font-black text-white transition hover:bg-emerald-400"
            >
              Browse All Coupons →
            </Link>
          </div>
      ...
      <section className="mt-10 overflow-hidden rounded-3xl bg-slate-900 px-6 py-8 text-center sm:px-10 sm:py-10">
        ...
        <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-slate-300">
          Browse more coupons, promo codes, and discounts available on
          DealPilot.
        </p>
